library flutter_google_places_nocors;

import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';
import 'package:flutter_google_places_nocors/model/place_details.dart';
import 'package:flutter_google_places_nocors/model/place_type.dart';
import 'package:flutter_google_places_nocors/model/prediction.dart';
...
class _GooglePlaceAutoCompleteTextFieldState
  ...
  getLocation(String text) async {
    ...
    try {
      ...
      if (kIsWeb) {
        final places = fgp.FlutterGooglePlacesSdk(widget.googleAPIKey);
        final predictionsResponse =
            await places.findAutocompletePredictions(text);

        if (predictionsResponse.predictions!.length > 0 &&
            (widget.textEditingController.text.toString().trim()).isNotEmpty) {
          List<Prediction> predictions =
              predictionsResponse.predictions!.map((predictionToChange) {
            return Prediction(
              description: predictionToChange.fullText,
              placeId: predictionToChange.placeId,
            );
          }).toList();
  ...
  Future<void> getPlaceDetailsFromPlaceId(Prediction prediction) async {
    ...
    try {
      if (kIsWeb) {
        final places = fgp.FlutterGooglePlacesSdk(widget.googleAPIKey);
        final placeResponse =
            await places.fetchPlace(prediction.placeId!, fields: [
          fgp.PlaceField.Location,
        ]);

        prediction.lat = placeResponse.place!.latLng!.lat.toString();
        prediction.lng = placeResponse.place!.latLng!.lng.toString();
      } else {
        var url =
